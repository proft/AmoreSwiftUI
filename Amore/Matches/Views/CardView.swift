...
struct CardView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            if fullScreenMode {
                FullScreenCardView(person: person, namespace: imageNamespace, fullscreenMode: $fullScreenMode)
                        ...
                        DragGesture()
                            .onChanged { value in
                                withAnimation(.default) {
                                    ...
                                    withAnimation {
                                        person.degree = (value.translation.width / 25) * -1
                                    }
                                }
                            }
                            .onEnded({ value in
...
struct CardView_Previews: PreviewProvider {
    static var previews: some View {
        CardView(person: .example, fullScreenMode: .constant(true))
    }
}
